...
try {
  ...
  for (let productIndex = 0; productIndex < products.length; productIndex++) {
    ...
    if (existing.rows[0]) {
      ...
      const currentProduct = await client.query("SELECT slug, category_id, name, description, image_url, additional_image_urls, published, featured, sort_order FROM website.shop_products WHERE id=$1", [productId]);
      const current = currentProduct.rows[0] || {};
      await client.query("UPDATE website.shop_products SET slug=$2,category_id=$3,name=$4,description=$5,image_url=$6,additional_image_urls=$7::jsonb,published=$8,featured=$9,sort_order=$10,source='shopify',source_id=$11,metadata=$12::jsonb,updated_at=now() WHERE id=$1", [productId, current.slug ?? productSlug, current.category_id ?? category.rows[0]?.id ?? null, current.name ?? product.title, current.description ?? text(product.body_html), current.image_url ?? images[0] ?? "", JSON.stringify(current.additional_image_urls ?? (images.length ? images.slice(1) : [])), current.published !== false, current.featured === true, Number.isFinite(Number(current.sort_order)) ? Number(current.sort_order) : productIndex * 10, String(product.id), JSON.stringify({ shopifyHandle: product.handle, shopifyProductType: product.product_type, shopifyTags: product.tags || [] })]);
    } else {
