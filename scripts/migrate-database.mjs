...
const schemaStatements = [
  ...
  `ALTER TABLE IF EXISTS website.shop_orders ADD COLUMN IF NOT EXISTS is_test_order boolean NOT NULL DEFAULT false`,
  `ALTER TABLE IF EXISTS website.shop_orders ADD COLUMN IF NOT EXISTS customer_acknowledgement_sent_at timestamptz`,
  `ALTER TABLE IF EXISTS website.shop_orders ADD COLUMN IF NOT EXISTS shipment_email_sent_at timestamptz`,
  `ALTER TABLE IF EXISTS website.shop_orders ADD COLUMN IF NOT EXISTS shipped_at timestamptz`,
  `ALTER TABLE IF EXISTS website.shop_orders ADD COLUMN IF NOT EXISTS shipment_tracking_url text`,
  `ALTER TABLE IF EXISTS website.shop_orders ADD COLUMN IF NOT EXISTS shipment_note text NOT NULL DEFAULT ''`,
...
try {
  ...
  const summary = await client.query("SELECT current_database() AS database, current_schema() AS schema");
  console.log("database.migrated", summary.rows[0]);
} catch (error) {
