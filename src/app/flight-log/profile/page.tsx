...
import { getFlightLogFriendSummary } from "@/lib/flight-log-social";
...
export default async function Page() {
  ...
  const [checkIns, friends] = await Promise.all([getFlightLogProfileSummary(customer.id), getFlightLogFriendSummary(customer.id)]);
  return <main className="flight-log-auth-page"><section><FlightLogProfileClient customer={customer} checkIns={checkIns} friends={friends} /><p className="flight-log-auth-links"><Link href="/flight-log">Back to Flight Log</Link><FlightLogSignOutButton /></p></section></main>;
}
