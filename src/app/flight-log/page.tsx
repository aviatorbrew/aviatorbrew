...
import { FlightLogPostModerationActions } from "@/components/flight-log/post-moderation-actions";
...
type Section = "home" | "my-posts" | "events" | "live-music" | "beer" | "locations";
...
const sections: Array<{ id: Section; label: string; href: string }> = [
  ...
  { id: "my-posts", label: "My Latest Posts", href: "/flight-log?section=my-posts" },
...
function OfficialCard({ post, customer }: { post: FlightLogPost; customer: Customer }) {
  return <article className="flight-log-activity-card">{post.imageUrl ? <img src={post.imageUrl} alt="" loading="lazy" /> : null}<div><p className="flight-log-activity-meta"><span>{post.isPinned ? "Pinned · " : ""}{flightLogCategoryLabels[post.category]}</span><strong>Official Aviator</strong></p><h2><Link href={`/flight-log/${post.slug}`}>{post.title}</Link></h2><p>{post.excerpt}</p><footer><time dateTime={post.publishedAt || post.updatedAt}>{postDate(post)}</time><span>Official dispatch</span></footer><FlightLogPostActions targetType="official" targetId={post.id} signedIn={Boolean(customer)} canInteract={canInteract(customer)} currentProfileId={customer?.id} currentRole={customer?.role} /><FlightLogPostModerationActions targetType="official" targetId={post.id} currentProfileId={customer?.id} currentRole={customer?.role} /></div></article>;
}
function CustomerCard({ post, customer }: { post: FlightLogCustomerPost; customer: Customer }) {
  return <article className="flight-log-activity-card">{post.media.map((item) => item.mediaType.startsWith("video/") ? <video key={item.url} src={item.url} controls preload="metadata" /> : <img key={item.url} src={item.url} alt="" loading="lazy" />)}<div><p className="flight-log-activity-meta"><span>Flight Crew Post</span><strong>@{post.authorHandle}</strong></p>{post.title ? <h2>{post.title}</h2> : null}<p>{post.body}</p>{post.taggedHandles.length ? <p className="flight-log-tags">With {post.taggedHandles.map((handle) => "@" + handle).join(", ")}</p> : null}<footer><time dateTime={post.createdAt}>{postDate(post)}</time><span>{post.authorName}</span></footer><FlightLogPostActions targetType="customer" targetId={String(post.id)} signedIn={Boolean(customer)} canInteract={canInteract(customer)} currentProfileId={customer?.id} currentRole={customer?.role} /><FlightLogPostModerationActions targetType="customer" targetId={post.id} authorProfileId={post.authorProfileId} authorRole={post.authorRole} currentProfileId={customer?.id} currentRole={customer?.role} /></div></article>;
}
function Feed({ posts, customerPosts, customer }: { posts: FlightLogPost[]; customerPosts: FlightLogCustomerPost[]; customer: Customer }) {
  const mixed = [...customerPosts.map((post) => ({ kind: "customer" as const, stamp: post.createdAt, post })), ...posts.map((post) => ({ kind: "official" as const, stamp: post.publishedAt || post.updatedAt, post }))].sort((a, b) => {
    if (a.kind !== b.kind) return a.kind === "customer" ? -1 : 1;
    if (a.kind === "official" && b.kind === "official") {
      const pinned = Number(b.post.isPinned) - Number(a.post.isPinned);
...
function MyPostsPanel({ posts, customer }: { posts: FlightLogCustomerPost[]; customer: Customer }) {
  if (!customer) return <div className="flight-log-data-panel"><h2>My latest posts</h2><p>Sign in to see your own Flight Log posts in one place.</p><Link className="button" href="/flight-log/sign-in">Sign In</Link></div>;
