...
export async function PATCH(request: NextRequest) {
  if (!isManager(request)) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  try {
    const body = await request.json() as { minimum?: number; priceCents?: number; bookingCutoffHours?: number; date?: string; time?: TourSlot; message?: string };
    if (body.minimum !== undefined || body.priceCents !== undefined || body.bookingCutoffHours !== undefined) {
      return NextResponse.json({ ok: true, ...(await setTourSettings({ minimum: body.minimum === undefined ? undefined : Number(body.minimum), priceCents: body.priceCents === undefined ? undefined : Number(body.priceCents), bookingCutoffHours: body.bookingCutoffHours === undefined ? undefined : Number(body.bookingCutoffHours) })) });
    }
