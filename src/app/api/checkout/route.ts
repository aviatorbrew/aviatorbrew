...
export async function POST(request: NextRequest) {
  try {
    ...
    const session = await createCheckoutSession({ item: body.item, quantity: body.quantity, customerEmail: body.email, referenceId: body.referenceId, metadata: body.metadata, origin: request.nextUrl.origin });
