...
export async function GET(_request: Request, { params }: { params: Promise<{ filename: string }> }) {
  ...
  for (const directory of [...new Set(directories())]) {
    try {
      ...
      return new NextResponse(new Uint8Array(file), { headers: { "Cache-Control": "public, max-age=31536000, immutable", "Content-Type": contentType, "X-Content-Type-Options": "nosniff" } });
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
  }
  return NextResponse.json({ error: "Shop image not found." }, { status: 404 });
}
