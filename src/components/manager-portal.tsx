...
function TourManager() {
  ...
  const [tourSettingsLoaded, setTourSettingsLoaded] = useState(false);
  const [minimum, setMinimum] = useState(DEFAULT_TOUR_MINIMUM);
  const [tourPrice, setTourPrice] = useState(DEFAULT_TOUR_PRICE_CENTS / 100);
  const [bookingCutoffHours, setBookingCutoffHours] = useState(DEFAULT_TOUR_BOOKING_CUTOFF_HOURS);
  function applyData(data: { signups: Signup[]; scheduledTours?: ScheduledTour[]; minimum?: number; priceCents?: number; bookingCutoffHours?: number }) { setSignups(data.signups); setScheduledTours(data.scheduledTours || []); setMinimum(data.minimum ?? DEFAULT_TOUR_MINIMUM); setTourPrice((data.priceCents ?? DEFAULT_TOUR_PRICE_CENTS) / 100); setBookingCutoffHours(data.bookingCutoffHours ?? DEFAULT_TOUR_BOOKING_CUTOFF_HOURS); setTourSettingsLoaded(true); }
  ...
  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    if (!tourSettingsLoaded) { setMessage("Tour settings are still loading. Try again after the configured values appear."); return; }
    setBusy(true);
    const response = await fetch("/api/manager/tours", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ minimum, priceCents: Math.round(tourPrice * 100), bookingCutoffHours }) });
    const body = await response.json(); setBusy(false);
    if (!response.ok) { setMessage(body.error); return; } applyData(body); setMessage("Tour settings saved. Public checkout, tour copy, and future emails now use $" + Number(body.priceCents / 100).toFixed(2) + " per guest, a " + body.minimum + "-guest launch threshold, and a " + body.bookingCutoffHours + "-hour booking cutoff.");
  }
...
function EventManager({ editId, returnTo }: { editId?: string; returnTo?: string } = {}) {
  ...
  function beginEdit(item: ManagedEvent) { setEditing(item); setAdding(false); setMessage("Editing " + item.title + "."); }
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setEvents(body.events || []); form.reset(); setAdding(false); setMessage("Special event saved.");
  ...
  async function update(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body = await readEventManagerResponse(response);
      setEvents(body.events || []); if (returnTo) returnFromManagerEdit(returnTo, "/manager/events"); setEditing(null); setMessage("Event details updated.");
    } catch (error) { setMessage(eventErrorMessage(error)); }
    finally { setBusy(false); }
  }
  ...
  return <section id="events" className="coupon-manager manager-events"><p className="eyebrow">Event operations</p><div className="manager-events-heading"><div><h2>Events</h2><p>Publish special events or live music to the database. Flight Log reads special events and live music from these manager-published rows.</p></div><button className="button" type="button" onClick={() => { setAdding((value) => !value); if (editing) setEditing(null); }} disabled={busy}>{adding ? "Close event form" : "Add an event"}</button></div><p className="media-message" role="status">{message}</p>
    {adding ? <form className="manager-event-form manager-event-add-form" onSubmit={create}><EventFormFields /><div className="manager-event-edit-actions"><button className="button" disabled={busy}>{busy ? "Saving..." : "Publish special event"}</button><button className="button button-outline" type="button" onClick={() => setAdding(false)} disabled={busy}>Cancel</button></div></form> : null}
...
type KegEditValues = {
  ...
  casePrice: string;
  case12Price: string;
  case12FourPackPrice: string;
  case12SixPackPrice: string;
  case16Price: string;
  case16FourPackPrice: string;
  case12Count: number;
  case16Count: number;
  caseCount: number;
  hidden: boolean;
...
function kegMoney(cents?: number) { return typeof cents === "number" && cents > 0 ? new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: cents % 100 === 0 ? 0 : 2, maximumFractionDigits: cents % 100 === 0 ? 0 : 2 }).format(cents / 100) : "-"; }
...
function kegJsonDollars(cents?: number) { return typeof cents === "number" && cents > 0 ? Number((cents / 100).toFixed(2)) : undefined; }
...
function kegPackPriceSummary(item: KegInventoryItem) {
  const case12Pack = item.case12FourPackPriceCents ? "12oz 4-pack " + kegMoney(item.case12FourPackPriceCents) : item.case12SixPackPriceCents ? "12oz 6-pack " + kegMoney(item.case12SixPackPriceCents) : "";
  return [
    case12Pack,
...
function downloadKegPdf(items: KegInventoryItem[], includeZeroInventory: boolean) {
  ...
  const lines = ["Aviator Brewing Company Keg Sales", "Exported " + created + (includeZeroInventory ? " - includes zero inventory" : " - available inventory only"), "", ...rows.map((item) => {
    const case12Pack = item.case12FourPackPriceCents ? "12oz 4-pack: " + pdfMoney(item.case12FourPackPriceCents) : item.case12SixPackPriceCents ? "12oz 6-pack: " + pdfMoney(item.case12SixPackPriceCents) : "";
    const packages = [
      ...
      item.case16FourPackPriceCents ? "16oz 4-pack: " + pdfMoney(item.case16FourPackPriceCents) : "",
    ].filter(Boolean).join(" | ");
...
function downloadKegJson(inventory: NonNullable<KegInventoryStatus>) {
  const payload = {
    ...
    instructions: "Edit price fields in dollars, save as JSON, then import it in Keg/package sales. For 12oz packages, set only one of case12FourPackPrice or case12SixPackPrice for the package actually sold. Use case16FourPackPrice for 16oz 4-packs. Keep beerName, sixthBblKegs, and fiftyLKegs on every row.",
    ...
    items: inventory.items.map((item) => ({
      ...
      case12FourPackPrice: kegJsonDollars(item.case12FourPackPriceCents),
      case12SixPackPrice: kegJsonDollars(item.case12SixPackPriceCents),
      cases16oz: item.case16Count || 0,
      case16ozPrice: kegJsonDollars(item.case16PriceCents),
      case16FourPackPrice: kegJsonDollars(item.case16FourPackPriceCents),
...
function KegInventoryManager({ editId, returnTo }: { editId?: string; returnTo?: string } = {}) {
  ...
  async function load() {
    ...
    const nextInventory = body.inventory || null;
    setInventory(nextInventory);
    if (editId && nextInventory) { const match = nextInventory.items.find((item: KegInventoryItem) => item.beerName === editId); if (match) beginEdit(match); }
  }
  useEffect(() => { load().catch((error) => setMessage(error.message)); }, []);
  ...
  async function saveEdit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/manager/kegs", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({
      ...
      case12FourPackPriceCents: kegCents(editForm.case12FourPackPrice),
      case12SixPackPriceCents: kegCents(editForm.case12SixPackPrice),
      case16PriceCents: kegCents(editForm.case16Price),
      case16FourPackPriceCents: kegCents(editForm.case16FourPackPrice),
    ...
    setInventory(body.inventory); if (returnTo) returnFromManagerEdit(returnTo, "/manager/kegs"); setEditing(null); setEditForm(null); setMessage(editForm.beerName + " keg details saved.");
  }
...
function BeerReleaseAlertManager({ editId, returnTo }: { editId?: string; returnTo?: string } = {}) {
  const [alerts, setAlerts] = useState<BeerReleaseAlert[]>([]);
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [adding, setAdding] = useState(false);
  async function load() { const response = await fetch("/api/manager/beer-release-alert", { cache: "no-store" }); const body = await response.json(); if (!response.ok) throw new Error(body.error); const nextAlerts: BeerReleaseAlert[] = body.alerts || []; setAlerts(nextAlerts); if (editId && nextAlerts.some((alert) => alert.id === editId)) setEditingId(editId); }
  useEffect(() => { load().catch((error) => setMessage(error.message)); }, [editId]);
  async function create(event: FormEvent<HTMLFormElement>) {
  ...
  async function save(event: FormEvent<HTMLFormElement>, id: string) {
    ...
    setAlerts(body.alerts || []); if (returnTo) returnFromManagerEdit(returnTo, "/manager/beer-release-alert"); setEditingId(null); setMessage("New Release Alert saved.");
  }
  ...
  return <section id="beer-release-alert" className="coupon-manager manager-beer-release-alert"><p className="eyebrow">Homepage alert</p><h2>New Release Alerts</h2><p>Show bold release notices directly under the scrolling banner. Use these for beer, liquor, THC soda, seltzer, or any other Aviator release.</p><p className="media-message" role="status">{message}</p>
    <div className="manager-beer-release-list"><h3 className="tour-signups-heading">Saved New Release Alerts</h3>{alerts.length ? alerts.map((alert) => { const visible = Boolean(alert.enabled && alert.beerName); const editing = editingId === alert.id; return <article className={"manager-beer-release-item" + (!visible ? " is-hidden" : "")} key={alert.id}>{editing ? <form key={alert.updatedAt || alert.id} className="manager-beer-release-form" onSubmit={(event) => save(event, alert.id)}>{fields(alert)}<div className="manager-beer-release-edit-actions"><button className="button" disabled={busy}>{busy ? "Saving..." : "Save New Release Alert"}</button><button className="button button-outline" type="button" onClick={() => returnTo ? returnFromManagerEdit(returnTo, "/manager/beer-release-alert") : setEditingId(null)} disabled={busy}>Cancel</button></div></form> : <div className="manager-beer-release-row"><div className="manager-beer-release-summary"><p className="eyebrow">{releaseStatus(alert)}</p><h3>{alert.beerName || "Untitled New Release"}</h3><p>{releaseWhen(alert)}</p><small>{alert.locations || "Locations not set"}{alert.specials ? " · " + alert.specials : ""}</small>{alert.sellSheetUrl ? <a href={alert.sellSheetUrl} target="_blank" rel="noreferrer">Open sell sheet</a> : <span>No sell sheet uploaded</span>}</div>{alert.sellSheetUrl ? <div className="manager-beer-release-thumb">{alert.sellSheetUrl.toLowerCase().endsWith(".pdf") ? <span>PDF</span> : <img src={alert.sellSheetUrl} alt="" />}</div> : null}<footer><a className="button" href={managerEditHref("beer-release-alert", alert.id)}>Edit</a><button type="button" onClick={() => setPublished(alert, !alert.enabled)} disabled={busy}>{alert.enabled ? "Unpublish" : "Publish"}</button><button type="button" onClick={() => remove(alert)} disabled={busy}>Delete</button></footer></div>}</article>; }) : <p>No New Release Alerts saved yet.</p>}<button className="button manager-beer-release-add" type="button" onClick={() => setAdding(true)} disabled={busy}>Add New Release</button></div>
...
type PortalBeverage = { id: string; source: "catalog" | "managed"; slug: string; name: string; category: "Soda" | "THC Soda" | "Seltzer"; description: string; note: string; image: string };

function BeerManager({ editId, returnTo }: { editId?: string; returnTo?: string } = {}) {
  const [beers, setBeers] = useState<PortalBeer[]>([]);
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState(false);
  const [editing, setEditing] = useState<PortalBeer | null>(null);
  async function load() { const response = await fetch("/api/manager/beers", { cache: "no-store" }); const body = await response.json(); if (!response.ok) throw new Error(body.error); const nextBeers: PortalBeer[] = body.beers || []; setBeers(nextBeers); if (editId) { const match = nextBeers.find((beer) => beer.id === editId); if (match) setEditing(match); } }
  useEffect(() => { load().catch((error) => setMessage(error.message)); }, [editId]);
  ...
  async function update(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) { setMessage(body.error); return; } setBeers(body.beers || []); if (returnTo) returnFromManagerEdit(returnTo, "/manager/beers"); setEditing(null); setMessage("Beer details updated.");
  }
...
function BeverageManager({ editId, returnTo }: { editId?: string; returnTo?: string } = {}) {
  const [beverages, setBeverages] = useState<PortalBeverage[]>([]);
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState(false);
  const [editing, setEditing] = useState<PortalBeverage | null>(null);
  const [adding, setAdding] = useState(false);
  async function load() { const response = await fetch("/api/manager/beyond-beer"); const body = await response.json(); if (!response.ok) throw new Error(body.error); const nextBeverages: PortalBeverage[] = body.beverages || []; setBeverages(nextBeverages); if (editId) { const match = nextBeverages.find((beverage) => beverage.id === editId); if (match) setEditing(match); } }
  useEffect(() => { load().catch((error) => setMessage(error.message)); }, [editId]);
  function beginEdit(beverage: PortalBeverage) { setEditing(beverage); setAdding(false); setMessage("Editing " + beverage.name + " in place."); }
  async function create(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true);
    ...
    if (!response.ok) { setMessage(body.error); return; } setBeverages(body.beverages || []); form.reset(); setAdding(false); setMessage("Beverage added to the public beverage menu.");
  }
  async function update(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true);
    ...
    if (!response.ok) { setMessage(body.error); return; } setBeverages(body.beverages || []); if (returnTo) returnFromManagerEdit(returnTo, "/manager/beverages"); setEditing(null); setMessage("Beverage details updated on the public beverage menu.");
  }
  ...
  return <section id="beverages" className="coupon-manager manager-beers"><p className="eyebrow">Beverage operations</p><div className="manager-beverage-heading"><div><h2>Soda, THC soda, and seltzer</h2><p>Edit the public beverage menu directly. Categories are kept separate as Soda, THC Soda, and Seltzer.</p></div><button className="button" type="button" onClick={() => { setAdding((value) => !value); if (editing) setEditing(null); }} disabled={busy}>{adding ? "Close beverage form" : "Add beverage"}</button></div><p className="media-message" role="status">{message}</p>
    {adding ? <form className="manager-beer-form" onSubmit={create}><label>Beverage name<input name="name" required maxLength={100} placeholder="Example: Aviator Cola" /></label><label>Category<select name="category" defaultValue="Soda"><option>Soda</option><option>THC Soda</option><option>Seltzer</option></select></label><label className="manager-beer-graphic">Beverage graphic<input name="graphic" type="file" accept=".png,.jpg,.jpeg,.webp,.pdf,image/png,image/jpeg,image/webp,application/pdf" required /><small>PNG, JPG, WEBP, or PDF - 25 MB max</small></label><label className="manager-beer-wide">Description<textarea name="description" required rows={3} maxLength={500} placeholder="Describe the product and flavor." /></label><label className="manager-beer-wide">Menu note<input name="note" required maxLength={160} placeholder="Example: Non-alcoholic - Family friendly" /></label><div className="manager-beer-edit-actions"><button className="button" disabled={busy}>{busy ? "Adding beverage..." : "Add beverage"}</button><button className="button button-outline" type="button" onClick={() => setAdding(false)} disabled={busy}>Cancel</button></div></form> : null}
...
function ManagerSectionContent({ section, editId, editType, returnTo }: { section: ManagerSection; editId?: string; editType?: string; returnTo?: string }) {
  switch (section) {
    case "newsletter": return <NewsletterManager />;
    case "tours": return <TourManager />;
    case "payments": return <PaymentTestManager />;
    case "locations": return <LocationManager editId={editId} returnTo={returnTo} />;
    case "coupons": return <div id="coupons"><CouponManager accessKey="manager-session" /></div>;
    case "beers": return <BeerManager editId={editId} returnTo={returnTo} />;
    case "shop": return <ShopManager editId={editId} editType={editType} returnTo={returnTo} />;
    case "beer-release-alert": return <BeerReleaseAlertManager editId={editId} returnTo={returnTo} />;
    case "flight-log": return <FlightLogAdmin mode={editId ? "edit" : "list"} postId={editId} returnTo={returnTo} />;
...
export function ManagerPortal({ section = "overview", editId, editType, returnTo }: { section?: ManagerSection; editId?: string; editType?: string; returnTo?: string }) {
  ...
  return <main className="manager-portal">
    ...
    <div className="content-wrap manager-page-shell">
      ...
      <div className="manager-route-content"><ManagerSectionContent section={section} editId={editId} editType={editType} returnTo={returnTo} /></div>
    </div>
  </main>;
}
