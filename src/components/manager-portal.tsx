...
type ManagedEvent = { id: string; title: string; date: string; startTime: string; endTime: string; location: string; description: string; ticketUrl: string; imageUrl?: string; galleryImages?: string[]; published: boolean; recurrence?: { frequency: string; interval: number; weekday?: number; ordinal?: number; endDate?: string } };
...
function EventManager() {
  ...
  return <section id="events" className="coupon-manager manager-events"><p className="eyebrow">Event operations</p><h2>Add a special event</h2><p>Publish non-music events to the Aviator Events page: beer releases, tastings, watch parties, campus gatherings, holiday events, and more. Live music continues to come from Aviator Live.</p><p className="media-message" role="status">{message}</p>
    ...
    <WebsitePhotosLibrary accessKey="manager-session" location={{ slug: "events", name: "Events Page" }} />
