    ...
    <div className="catering-order-summary" aria-live="polite">
      ...
      {selectedRows.length ? <>
        <ul>{selectedRows.map((row) => {
          const unitCents = (row.priceCents || 0) + row.optionPriceCents;
          const lineCents = unitCents * row.quantity;
          return <li key={row.id}><strong>{row.quantity} x {row.name}</strong><span>{row.group}{row.option ? " - " + row.option : ""}{row.note ? " - " + row.note : ""}{row.priceCents ? " - " + money(lineCents) : " - price to confirm"}</span></li>;
        })}</ul>
