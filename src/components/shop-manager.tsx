...
function ProductForm({ product, categories, busy, onSubmit, onCancel, imageVersion }: { product?: ShopProduct | null; categories: ShopCategory[]; busy: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void; imageVersion: number }) {
  ...
  useEffect(() => { setVariants(product?.variants.length ? product.variants.map(variantFromProduct) : [blankVariant()]); }, [product?.id]);
  useEffect(() => () => { if (previewUrl) URL.revokeObjectURL(previewUrl); }, [previewUrl]);
  const displayedImage = previewUrl || (product?.imageUrl ? imageWithVersion(product.imageUrl, imageVersion) : "");
  ...
  return <form className="manager-shop-product-form" onSubmit={onSubmit}>
    ...
    <label>Product image<input name="image" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.currentTarget.files?.[0]; setPreviewUrl((current) => { if (current) URL.revokeObjectURL(current); return file ? URL.createObjectURL(file) : ""; }); }} /><small>{product?.imageUrl ? "Choose a new file to replace the current image, or leave blank to keep it." : "JPG, PNG, or WEBP up to 10 MB."}</small></label>
    {displayedImage ? <div className="manager-shop-current-image"><img src={displayedImage} alt="" /><span>{previewUrl ? "New image preview" : "Current image"}</span></div> : null}
...
export function ShopManager({ editId, editType, returnTo }: { editId?: string; editType?: string; returnTo?: string } = {}) {
  ...
  async function saveCategory(event: FormEvent<HTMLFormElement>) {
    ...
    if (ok) { if (returnTo) returnFromManagerEdit(returnTo, "/manager/shop"); setEditingCategory(null); form.reset(); }
  }
  ...
  return <section id="shop" className="coupon-manager manager-shop"><p className="eyebrow">ShopNew operations</p><h2>Aviator ShopNew</h2><p>Manage the dark storefront, full cart, Stripe orders, product catalogs, USPS parcel defaults, and automatic order bonus.</p><p className="media-message" role="status">{message}</p>
    <div className="manager-shop-layout"><aside className="manager-shop-catalogs"><h3>Catalogs</h3><div className="manager-shop-filter"><button type="button" className={activeCategory === "all" ? "is-active" : ""} onClick={() => setActiveCategory("all")}>All</button>{catalog.categories.map((category) => <button type="button" className={activeCategory === category.slug ? "is-active" : ""} onClick={() => setActiveCategory(category.slug)} key={category.id}>{category.name}</button>)}</div><form className="manager-shop-category-form" onSubmit={saveCategory}>{editingCategory ? <input type="hidden" name="id" value={editingCategory.id} /> : null}<label>Catalog name<input name="name" required maxLength={80} defaultValue={editingCategory?.name || ""} placeholder="Apparel" /></label><label>Description<input name="description" maxLength={160} defaultValue={editingCategory?.description || ""} placeholder="T-shirts, hoodies, and crew gear" /></label><label>Sort order<input name="sortOrder" type="number" defaultValue={editingCategory?.sortOrder || 0} /></label><label className="manager-event-publish"><input name="published" type="checkbox" defaultChecked={editingCategory ? editingCategory.published : true} /> Publish catalog</label><div><button className="button" disabled={busy}>{busy ? "Saving..." : editingCategory ? "Save catalog" : "Add catalog"}</button>{editingCategory ? <button className="button button-outline" type="button" onClick={() => returnTo ? returnFromManagerEdit(returnTo, "/manager/shop") : setEditingCategory(null)} disabled={busy}>Cancel</button> : null}</div></form>{catalog.categories.length ? <ul>{catalog.categories.map((category) => <li key={category.id}><span>{category.name}</span><div><a className="button" href={managerEditHref("shop", String(category.id), "category")}>Edit</a><button type="button" onClick={() => removeCategory(category)} disabled={busy}>Delete</button></div></li>)}</ul> : null}</aside>
      ...
      <label>Order notification email<input name="orderNotificationEmail" type="email" defaultValue={settings.orderNotificationEmail} required autoComplete="email" /><small>Paid ShopNew order details are sent here after Stripe confirms payment.</small></label>
