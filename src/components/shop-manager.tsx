...
<label>Product type<select name="productType" defaultValue={product?.productType || initialProductType} onChange={(event) => setProductType(event.currentTarget.value === "ticket" ? "ticket" : "merchandise")}><option value="merchandise">Merchandise</option><option value="ticket">Event ticket</option></select><small>Tickets use limited inventory, do not require shipping, and create a purchaser roster.</small></label>
    ...
    {displayedImages.length ? <div className="manager-shop-current-images manager-shop-wide">{displayedImages.map((image, index) => <div className="manager-shop-current-image" key={image}><ShopProductGallery images={[image]} name={(product?.name || "Shop product") + " photo " + (index + 1)} /><span>{index === 0 ? "Main photo" : image.startsWith("blob:") ? "New photo" : "Gallery photo"}</span>{!image.startsWith("blob:") ? <label><input name="removeImages" type="checkbox" value={currentImages[index] || ""} /> Remove</label> : null}</div>)}</div> : null}
