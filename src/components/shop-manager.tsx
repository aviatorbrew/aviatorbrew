...
function ProductForm({ product, initialProductType = "merchandise", categories, locations, busy, onSubmit, onCancel, imageVersion }: { product?: ShopProduct | null; initialProductType?: "merchandise" | "ticket"; categories: ShopCategory[]; locations: NonNullable<ShopCatalog["locations"]>; busy: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void; imageVersion: number }) {
  ...
  const [productType, setProductType] = useState<"merchandise" | "ticket">(product?.productType || initialProductType);
  ...
  useEffect(() => { setVariants(product?.variants.length ? product.variants.map(variantFromProduct) : [blankVariant()]); setPreviewUrls([]); setEditingVariantIndex(null); setProductType(product?.productType || initialProductType); }, [product?.id, initialProductType]);
...
<label>Product type<select name="productType" defaultValue={product?.productType || initialProductType} onChange={(event) => setProductType(event.currentTarget.value === "ticket" ? "ticket" : "merchandise")}><option value="merchandise">Merchandise</option><option value="ticket">Event ticket</option></select><small>Tickets use limited inventory, do not require shipping, and create a purchaser roster.</small></label>
    ...
    {productType === "ticket" ? <section className="manager-shop-ticket-fields manager-shop-wide"><div><p className="eyebrow">Ticket event details</p><h4>Admission setup</h4><p>Inventory across ticket options is the event capacity. Sales close automatically at the sales end time or event start.</p></div><label className="manager-event-publish manager-shop-ticket-prominence"><input name="published" type="checkbox" defaultChecked={product ? product.published : true} /><span><strong>Publish product</strong><small>Leave this unchecked until the ticket is ready for sale. Draft tickets are hidden from Aviator Supply and website Events.</small></span></label><label>Event location<select name="ticketLocationSlug" required defaultValue={product?.ticketLocationSlug || ""}><option value="">Choose a location</option>{locations.map((location) => <option value={location.slug} key={location.slug}>{location.name}</option>)}</select></label><label>Event date and time<input name="ticketEventStartsAt" type="datetime-local" required defaultValue={localDateTime(product?.ticketEventStartsAt)} /></label><label>Total tickets for sale<input name="ticketCapacity" type="number" min="1" step="1" required defaultValue={product?.ticketCapacity || 100} /><small>This is the total event capacity across every ticket option.</small></label><label>Maximum per customer<input name="ticketMaxPerOrder" type="number" min="1" step="1" required defaultValue={product?.ticketMaxPerOrder || 20} /><small>This is the total number one customer can buy for this event, including previous orders.</small></label><label className="manager-event-publish manager-shop-ticket-prominence"><input name="ticketFullWidth" type="checkbox" defaultChecked={product?.ticketFullWidth || false} /><span><strong>Prominent full-width display</strong><small>Span the entire shop product grid for a featured special event.</small></span></label><label className="manager-event-publish manager-shop-ticket-prominence"><input name="ticketPublishAsEvent" type="checkbox" defaultChecked={product?.ticketPublishAsEvent || false} /><span><strong>Publish in website Events</strong><small>Show this ticket first above all other special events on the Events page.</small></span></label><label>Sales end <small>(optional)</small><input name="ticketSalesEndAt" type="datetime-local" defaultValue={localDateTime(product?.ticketSalesEndAt)} /></label></section> : null}
    ...
    <label className="manager-shop-wide">Description<textarea name="description" rows={productType === "ticket" ? 12 : 3} maxLength={productType === "ticket" ? 10000 : 900} defaultValue={product?.description || ""} placeholder="Describe the item, color, material, or pickup notes." /><small>{productType === "ticket" ? "Up to 10,000 characters. Line breaks and blank lines are preserved." : "Up to 900 characters."}</small></label>
    ...
    {displayedImages.length ? <div className="manager-shop-current-images manager-shop-wide">{displayedImages.map((image, index) => <div className="manager-shop-current-image" key={image}><ShopProductGallery images={[image]} name={(product?.name || "Shop product") + " photo " + (index + 1)} /><span>{index === 0 ? "Main photo" : image.startsWith("blob:") ? "New photo" : "Gallery photo"}</span>{!image.startsWith("blob:") ? <div className="manager-shop-image-actions"><label><input name="mainImageUrl" type="radio" value={currentImages[index] || ""} defaultChecked={index === 0} /> Main</label><label><input name="removeImages" type="checkbox" value={currentImages[index] || ""} /> Remove</label></div> : null}</div>)}</div> : null}
    <section className="manager-shop-variant-editor manager-shop-wide">
      ...
      {variants.map((variant, index) => <article className={editingVariantIndex === index ? "manager-shop-variant-row is-expanded" : "manager-shop-variant-row"} key={index}>
        {variant.id ? <input type="hidden" name={"variantId_" + index} value={variant.id} /> : null}
        <header>
        ...
        {productType !== "ticket" ? <>        <section className="manager-shop-variant-section">
        ...
        </section></> : <section className="manager-shop-variant-section manager-shop-ticket-option-note"><div className="manager-shop-variant-section-title"><span>3</span><div><h4>Ticket inventory</h4><p>Total event capacity and the per-customer limit are controlled in Ticket event details above. Ticket options only need a name, SKU, price, and sale status.</p></div></div></section>}
        </div>
      </article>)}
    </section>
    {productType !== "ticket" ? <label className="manager-event-publish"><input name="published" type="checkbox" defaultChecked={product ? product.published : true} /> Publish product</label> : null}
...
export function ShopManager({ editId, editType, returnTo }: { editId?: string; editType?: string; returnTo?: string } = {}) {
  ...
  const [addingProductType, setAddingProductType] = useState<"merchandise" | "ticket">("merchandise");
  ...
  return <section id="shop" className="coupon-manager manager-shop"><p className="eyebrow">Aviator Supply operations</p><h2>Aviator Supply</h2><p>Manage the dark storefront, full cart, Stripe orders, product catalogs, USPS parcel defaults, and automatic order bonus.</p><p className="media-message" role="status">{message}</p>
    <div className="manager-shop-layout"><aside className="manager-shop-catalogs"><div className="manager-shop-catalog-heading"><h3>Catalogs</h3><button className="button" type="button" onClick={() => { setAddingCategory(true); setEditingCategory(null); }} disabled={busy || addingCategory}>Add catalog</button></div><div className="manager-shop-filter"><button type="button" className={activeCategory === "all" ? "is-active" : ""} onClick={() => setActiveCategory("all")}>All</button>{catalog.categories.map((category) => <button type="button" className={activeCategory === category.slug ? "is-active" : ""} onClick={() => setActiveCategory(category.slug)} key={category.id}>{category.name}</button>)}</div>{addingCategory || editingCategory ? <form key={editingCategory ? "category-" + editingCategory.id : "new-category"} className="manager-shop-category-form" onSubmit={saveCategory}>{editingCategory ? <input type="hidden" name="id" value={editingCategory.id} /> : null}<label>Catalog name<input name="name" required maxLength={80} defaultValue={editingCategory?.name || ""} placeholder="Apparel" /></label><label>Description<input name="description" maxLength={160} defaultValue={editingCategory?.description || ""} placeholder="T-shirts, hoodies, and crew gear" /></label><label>Sort order<input name="sortOrder" type="number" defaultValue={editingCategory?.sortOrder || 0} /></label><label className="manager-event-publish"><input name="published" type="checkbox" defaultChecked={editingCategory ? editingCategory.published : true} /> Publish catalog</label><div><button className="button" disabled={busy}>{busy ? "Saving..." : editingCategory ? "Save catalog" : "Add catalog"}</button><button className="button button-outline" type="button" onClick={() => returnTo ? returnFromManagerEdit(returnTo, "/manager/shop") : (() => { setEditingCategory(null); setAddingCategory(false); })()} disabled={busy}>Cancel</button></div></form> : null}{catalog.categories.length ? <ul>{catalog.categories.map((category, index) => <li key={category.id}><span><strong>{category.name}</strong><small>Order {category.sortOrder}</small></span><div><button type="button" onClick={() => moveCategory(category, -1)} disabled={busy || index === 0}>Up</button><button type="button" onClick={() => moveCategory(category, 1)} disabled={busy || index === catalog.categories.length - 1}>Down</button><a className="button" href={managerEditHref("shop", String(category.id), "category")}>Edit</a><button type="button" onClick={() => removeCategory(category)} disabled={busy}>Delete</button></div></li>)}</ul> : null}</aside>
      <div className="manager-shop-main"><div className="manager-shop-product-toolbar"><div><p className="eyebrow">Products</p><h3>Shop items</h3></div><div className="manager-shop-product-actions"><button className="button" type="button" onClick={() => { setAddingProductType("merchandise"); setAddingProduct(true); setEditingProduct(null); }} disabled={busy || addingProduct}>Add new item</button><button className="button button-outline" type="button" onClick={() => { setAddingProductType("ticket"); setAddingProduct(true); setEditingProduct(null); }} disabled={busy || addingProduct}>Add event ticket</button></div></div>
    ...
    {ticketProducts.length ? <section className="manager-shop-ticket-rosters"><div className="manager-shop-form-heading"><div><p className="eyebrow">Event admission</p><h3>Ticket purchaser lists</h3></div></div>{ticketProducts.map((product) => {
      ...
      const sold = product.ticketSoldCount;
      const reserved = product.ticketReservedCount;
      return <article className="manager-shop-ticket-roster" key={product.id}><header><div><p className="eyebrow">{product.ticketLocationName || "Location not set"}</p><h4>{product.name}</h4><p>{dateTime(product.ticketEventStartsAt)} - {sold} sold of {product.ticketCapacity} - {reserved} reserved - {product.ticketAvailableCount} available now - limit {product.ticketMaxPerOrder} per customer</p></div><a className="button button-outline" href={"/api/manager/shop/tickets?productId=" + product.id}>Download CSV</a></header>{purchases.length ? <div className="manager-shop-ticket-table"><table><thead><tr><th>Name</th><th>Email</th><th>Ticket</th><th>Party</th><th>Purchased</th></tr></thead><tbody>{purchases.map((purchase) => <tr key={purchase.id}><td>{purchase.purchaserName}</td><td><a href={"mailto:" + purchase.purchaserEmail}>{purchase.purchaserEmail}</a></td><td>{purchase.variantLabel}</td><td>{purchase.partySize}</td><td>{dateTime(purchase.paidAt)}</td></tr>)}</tbody></table></div> : <p className="tour-schedule-empty">No paid ticket purchasers yet.</p>}</article>;
    })}</section> : null}
