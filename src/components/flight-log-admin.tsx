...
import type { FlightLogCustomerPost } from "@/lib/flight-log-social";
import { FlightLogUsersAdmin } from "@/components/flight-log-users-admin";

type Option = { id: string; label: string };
type Options = { categories: Option[]; statuses: FlightLogStatus[]; locations: Option[]; events: Option[]; beers: Option[] };
type Props = { mode?: "list" | "new" | "edit"; postId?: string; returnTo?: string };
...
function customerSearchText(post: FlightLogCustomerPost) { return [post.title, post.body, post.authorName, post.authorHandle, post.status, post.visibility, ...post.taggedHandles].join(" ").toLowerCase(); }
function customerStamp(post: FlightLogCustomerPost) { return new Date(post.createdAt).getTime(); }
...
export function FlightLogAdmin({ mode = "list", postId, returnTo }: Props) {
  const [posts, setPosts] = useState<FlightLogPost[]>([]);
  const [customerPosts, setCustomerPosts] = useState<FlightLogCustomerPost[]>([]);
  ...
  const [workspace, setWorkspace] = useState<"posts" | "users">("posts");
  ...
  const customerFiltered = useMemo(() => {
    ...
    return [...customerPosts]
      .filter((post) => (statusFilter === "all" || post.status === statusFilter) && (!needle || customerSearchText(post).includes(needle)))
      .sort((a, b) => customerStamp(b) - customerStamp(a));
  }, [customerPosts, query, statusFilter]);
  ...
  async function load() {
    ...
    setPosts(body.posts || []);
    setCustomerPosts(body.customerPosts || []);
  ...
  function showList() { if (returnTo) returnFromManagerEdit(returnTo, "/manager/flight-log"); else { setScreenMode("list"); setActivePostId(undefined); setDraft(blank); } }
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setPosts(body.posts || []); if (Array.isArray(body.customerPosts)) setCustomerPosts(body.customerPosts); setDraft(body.post || blank);
      if (returnTo && draft.id) returnFromManagerEdit(returnTo, "/manager/flight-log");
  ...
  async function quickUpdate(post: FlightLogPost, patch: Partial<FlightLogPost>) {
    ...
    try {
      const response = await fetch("/api/manager/flight-log", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: post.id, ...patch }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Could not update post.");
      setPosts(body.posts || []); if (Array.isArray(body.customerPosts)) setCustomerPosts(body.customerPosts); setMessage("Post updated.");
  ...
  async function archive(post: FlightLogPost) {
    ...
    try {
      ...
      setPosts(body.posts || []); if (Array.isArray(body.customerPosts)) setCustomerPosts(body.customerPosts); setMessage("Post archived.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Could not archive post."); }
    finally { setBusy(false); }
  }

  async function deletePost(post: FlightLogPost) {
    if (!window.confirm("Delete this Flight Log post permanently? This cannot be undone.")) return;
    setBusy(true); setMessage("Deleting post...");
    try {
      const response = await fetch("/api/manager/flight-log?id=" + encodeURIComponent(post.id) + "&delete=true", { method: "DELETE" });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Could not delete post.");
      setPosts(body.posts || []); if (Array.isArray(body.customerPosts)) setCustomerPosts(body.customerPosts); setMessage("Post deleted.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Could not delete post."); }
    finally { setBusy(false); }
  }

  async function deleteCustomerPost(post: FlightLogCustomerPost) {
    const label = post.title || post.body.slice(0, 60) || "this customer post";
    if (!window.confirm("Delete this customer Flight Log post permanently? This cannot be undone.\n\n" + label)) return;
    setBusy(true); setMessage("Deleting customer post...");
    try {
      const response = await fetch("/api/manager/flight-log?customerPostId=" + encodeURIComponent(String(post.id)), { method: "DELETE" });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Could not delete customer post.");
      setPosts(body.posts || []); setCustomerPosts(body.customerPosts || []); setMessage("Customer post deleted.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Could not delete customer post."); }
    finally { setBusy(false); }
  }
  ...
  return <section className="manager-page-shell flight-log-admin"><div className="content-wrap">
    <header className="manager-topbar"><div><p className="eyebrow">Official dispatches</p><h1>Flight Log Admin</h1><p>Create, preview, publish, pin, unpublish, archive, and choose homepage Flight Log posts.</p></div><div><Link className="button button-outline" href="/flight-log">Public feed <ArrowUpRight /></Link><button className="button" type="button" onClick={() => { setWorkspace("posts"); setScreenMode("new"); setActivePostId(undefined); setDraft(blank); }}>New post</button></div></header>
    <nav className="flight-log-admin-tabs" aria-label="Flight Log administration">
      <button type="button" className={workspace === "posts" ? "is-active" : ""} aria-current={workspace === "posts" ? "page" : undefined} onClick={() => { setWorkspace("posts"); setScreenMode("list"); }}>Posts</button>
      <button type="button" className={workspace === "users" ? "is-active" : ""} aria-current={workspace === "users" ? "page" : undefined} onClick={() => setWorkspace("users")}>Users</button>
    </nav>
    {workspace === "users" ? <FlightLogUsersAdmin /> : <>
    ...
    {screenMode === "list" ? <>
      ...
      <div className="flight-log-admin-list">{filtered.length ? filtered.map((post) => <article key={post.id}><div><p className="flight-log-admin-row-flags"><StatusChip status={post.status} />{post.isPinned ? <span>Pinned</span> : null}{post.showOnHomepage ? <span>Homepage</span> : null}<span>{flightLogCategoryLabels[post.category]}</span></p><h2>{post.title}</h2><p>{post.excerpt}</p><small>{post.authorName} - {post.publishedAt ? new Date(post.publishedAt).toLocaleString() : "Not published"}</small></div>{post.imageUrl ? <img src={post.imageUrl} alt="" /> : null}<footer className="flight-log-admin-actions" aria-label={"Actions for " + post.title}><a className="button" href={managerEditHref("flight-log", post.id)}>Edit</a>{post.status !== "published" ? <button type="button" onClick={() => quickUpdate(post, { status: "published" })} disabled={busy}>Publish</button> : <button type="button" onClick={() => quickUpdate(post, { status: "draft" })} disabled={busy}>Unpublish</button>}<button type="button" onClick={() => quickUpdate(post, { isPinned: !post.isPinned })} disabled={busy}>{post.isPinned ? "Unpin" : "Pin"}</button><button type="button" onClick={() => quickUpdate(post, { showOnHomepage: !post.showOnHomepage })} disabled={busy || post.status !== "published"}>{post.showOnHomepage ? "Remove from homepage" : "Feature on homepage"}</button>{post.status !== "archived" ? <button type="button" onClick={() => archive(post)} disabled={busy}>Archive</button> : null}<button className="is-danger" type="button" onClick={() => deletePost(post)} disabled={busy}>Delete</button></footer></article>) : <p className="flight-log-empty">No posts match this filter.</p>}</div>
      <section className="flight-log-admin-customer-posts" aria-labelledby="customer-flight-log-posts"><header><div><p className="eyebrow">Customer activity</p><h2 id="customer-flight-log-posts">Customer posts ({customerFiltered.length})</h2></div><p>Admin and moderator cleanup for Flight Log posts created by customers.</p></header><div className="flight-log-admin-list">{customerFiltered.length ? customerFiltered.map((post) => { const image = post.media.find((item) => item.mediaType.startsWith("image/"))?.url; return <article key={post.id}><div><p className="flight-log-admin-row-flags"><span>Customer post</span><span>{statusLabel(post.status)}</span><span>{post.visibility}</span>{post.authorRole !== "user" ? <span>{statusLabel(post.authorRole)}</span> : null}</p><h2>{post.title || "Untitled customer post"}</h2><p>{post.body}</p><small>{post.authorName}{post.authorHandle ? " @" + post.authorHandle : ""} - {post.createdAt ? new Date(post.createdAt).toLocaleString() : "No date"}</small>{post.taggedHandles.length ? <small>Tagged: {post.taggedHandles.map((handle) => "@" + handle).join(", ")}</small> : null}</div>{image ? <img src={image} alt="" /> : null}<footer className="flight-log-admin-actions" aria-label={"Actions for customer post " + post.id}><button className="is-danger" type="button" onClick={() => deleteCustomerPost(post)} disabled={busy}>Delete</button></footer></article>; }) : <p className="flight-log-empty">No customer posts match this filter.</p>}</div></section>
