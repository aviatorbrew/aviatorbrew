...
export function ShopCartCheckout({ checkoutStatus }: { checkoutStatus?: string }) {
  ...
  async function checkout() {
    if (!selectedToken) { setState("error"); setMessage("Calculate and choose a shipping rate first."); return; }
    setState("checkout"); setMessage("");
    try {
      const response = await fetch("/api/shop/checkout", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ items: requestItems, email, phone: address.phone, address, shippingToken: selectedToken }) });
  ...
  return <section className="section shop-cart-page"><div className="content-wrap">
    ...
    <div className="shop-checkout-grid"><div className="shop-cart-manifest">
      ...
      <div className={"shop-bonus-panel " + (cart?.bonusItem ? "is-earned" : "")}>
        <span aria-hidden="true">{cart?.bonusItem ? "BONUS LOADED" : "BONUS APPROACH"}</span>
        <div><strong>{cart?.bonusItem ? cart.bonusItem.productName : "Free Aviator sticker over " + money(threshold)}</strong><p>{cart?.bonusItem ? "Added automatically at no charge." : "Add " + money(bonusRemaining) + " more in merchandise to unlock it."}</p></div>
      </div>
    ...
    <form className="shop-shipping-form" onSubmit={calculateShipping}>
      ...
      <label>Apartment, suite, etc. <small>(optional)</small><input value={address.street2} onChange={(event) => addressField("street2", event.currentTarget.value)} autoComplete="shipping address-line2" /></label>
      <div className="shop-address-row"><label>City<input required value={address.city} onChange={(event) => addressField("city", event.currentTarget.value)} autoComplete="shipping address-level2" /></label><label>State<input required maxLength={2} value={address.state} onChange={(event) => addressField("state", event.currentTarget.value)} autoComplete="shipping address-level1" /></label><label>ZIP<input required value={address.zip} onChange={(event) => addressField("zip", event.currentTarget.value)} autoComplete="shipping postal-code" /></label></div>
      <label>Phone<input type="tel" value={address.phone} onChange={(event) => addressField("phone", event.currentTarget.value)} autoComplete="tel" /></label>
      <button className="button button-outline" disabled={state === "shipping"}>{state === "shipping" ? "Calling USPS..." : "Calculate USPS shipping"}</button>
      ...
      <dl className="shop-order-totals"><div><dt>Merchandise</dt><dd>{money(cart?.subtotalCents || 0)}</dd></div><div><dt>Shipping</dt><dd>{shipping ? money(shipping.amountCents) : "Calculate above"}</dd></div><div><dt>Total</dt><dd>{shipping ? money(total) : "--"}</dd></div></dl>
      <button className="button shop-checkout-button" type="button" onClick={checkout} disabled={!selectedToken || state === "checkout"}>{state === "checkout" ? "Opening secure checkout..." : "Checkout"}</button>
      <p className="shop-secure-note">ZIP-based rates are estimates until live USPS rates are connected. Prices and inventory are checked again before payment. Stripe securely handles card details.</p>
    </form></div>
  </div></section>;
}
