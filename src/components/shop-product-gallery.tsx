...
export function ShopProductGallery({ images, name }: { images: string[]; name: string }) {
  ...
  const lightbox = open ? createPortal(<div className="beer-image-lightbox shop-image-lightbox" role="dialog" aria-modal="true" aria-label={name + " full-screen product image"}>
    <header><div><p>Aviator Supply</p><h2>{name}</h2></div><button ref={closeButton} className="beer-lightbox-close" type="button" onClick={() => setOpen(false)} aria-label="Close full-screen product image">×</button></header>
  ...
  return <div className={images.length > 1 ? "shop-product-gallery has-thumbnails" : "shop-product-gallery"}><button className="shop-new-image shop-image-trigger" type="button" onClick={openViewer} aria-label={"View " + name + " product image full size"}><img src={active} alt={name} /><span className="beer-image-expand" aria-hidden="true" data-tooltip="View full size">↗</span></button>{images.length > 1 ? <div className="shop-product-thumbs" aria-label={name + " photos"}>{images.map((image, index) => <button type="button" className={active === image ? "is-active" : ""} aria-pressed={active === image} onClick={() => setActive(image)} aria-label={"Show photo " + (index + 1)} key={image}><img src={image} alt="" /></button>)}</div> : null}{lightbox}</div>;
}
