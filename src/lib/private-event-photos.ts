import { getFeaturedPhotos, listUploadedPhotos, type PhotoSource } from "@/lib/website-photo-storage";

export type PrivateEventPhoto = { name: string; size: number; updatedAt: string; url: string; source: PhotoSource; featured?: boolean };
...
export async function getPrivateEventPhotos(): Promise<PrivateEventPhoto[]> {
  ...
  const photos = uploads.map((photo) => ({ ...photo, featured: Boolean(selected && photo.name === selected.name && photo.source === selected.source) }));
