...
export type BreweryPhoto = { name: string; size: number; updatedAt: string; url: string; source: PhotoSource; mediaType: WebsiteMediaType; featured?: boolean };
...
export async function getBreweryPhotos(): Promise<BreweryPhoto[]> {
  ...
  const photos = [...uploads, ...bundledPhotos().filter((photo) => !uploadedNames.has(photo.name) && !hiddenNames.has(photo.name))]
    .map((photo) => ({ ...photo, featured: Boolean(selected && photo.name === selected.name && photo.source === selected.source && photo.mediaType === "image") }));
...
export async function getBreweryHero() {
  ...
  const photo = photos.find((item) => item.mediaType === "image");
